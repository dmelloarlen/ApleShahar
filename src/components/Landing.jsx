...
import { TrendingUp, CheckCircle, Leaf, MapPin, Building2, ArrowRight, LogOut, LayoutDashboard, PlusCircle, HeartHandshake } from 'lucide-react';
    ...
    <div className="min-h-screen bg-white text-stone-900 font-sans selection:bg-blue-100">
          ...
          <div className="flex items-center gap-8">
            ...
            {user && (
              <div className="hidden md:flex items-center gap-2 border-l border-stone-200 pl-8">
                {user.role === 'citizen' ? (
                  <>
                    <Link to="/dashboard/report-issue" className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors border border-transparent text-stone-600 hover:bg-stone-100 hover:border-stone-200">
                      <PlusCircle className="w-4 h-4" /> Report Issue
                    </Link>
                    <Link to="/dashboard/manage-complaints" className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors border border-transparent text-stone-600 hover:bg-stone-100 hover:border-stone-200">
                      <HeartHandshake className="w-4 h-4" /> My Complaints
                    </Link>
                    <Link to="/dashboard/facility-requests" className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors border border-transparent text-stone-600 hover:bg-stone-100 hover:border-stone-200">
      ...
      {user && (
        <div className="md:hidden bg-white border-b border-stone-200 px-4 py-3 flex gap-2 overflow-x-auto shadow-sm">
          {user.role === 'citizen' ? (
            <>
              <Link to="/dashboard/report-issue" className="whitespace-nowrap shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors border border-transparent text-stone-600 hover:bg-stone-100">
                <PlusCircle className="w-4 h-4" /> Report Issue
              </Link>
              <Link to="/dashboard/manage-complaints" className="whitespace-nowrap shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors border border-transparent text-stone-600 hover:bg-stone-100">
                <HeartHandshake className="w-4 h-4" /> My Complaints
              </Link>
              <Link to="/dashboard/facility-requests" className="whitespace-nowrap shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-colors border border-transparent text-stone-600 hover:bg-stone-100">
                <Leaf className="w-4 h-4" /> Facility Requests
              </Link>
            </>
          ) : (
